...
public class RecordImplAgent extends Record{
	...
	public void changeCommissionCodeApprovedWorkflow(String userFrank,String frankPass,String userJennifer,String jenniferPass,String container,String model,String entity,String Identifie,String agentID,String Firstname,String FirstnameValue,String Lastname,String LastnameValue,String CommissionCode,String CommissionCodeValue,String StartDate,String StartDateValue){
		...
		logger.warn("for agent :"+agentID+"The initial commission code is:"+initialCode+",the initial status is:"+initialStatus);
		...
		logger.warn("beforeProcess action entry for frank is:"+beforeProcess);
		logger.warn("beforeProcess update entry for administrator is:"+beforeProcessUpdateAdministrator);
		this.closeJournal();
		...
		logger.warn("after process done ,status expected is pending, infact it is :"+statusBeforeSubmitAfterProcess);
		...
		logger.warn("afterProcess action entry for frank is :"+afterProcess);
		logger.warn("afterProcess update entry for administrator is :"+afterProcessUpdateAdministrator);
	...
	//	Assert.assertFalse(this.getElementByXpath(locator.getString("xpath.record.agent.comchangeworkflow.taskopened.identifer.input")).isEnabled(), "frank can not modify identifer in opened task test failed!");
		...
		logger.warn("beforeApprove ,the update entry for administrator is :"+beforeApprove);
		this.closeJournal();
	    ...
	    logger.warn("afterapproved ,the commission code is:"+codeApproved);
	//    Assert.assertTrue(price.equals(priceSubmited));
		...
		logger.warn("afterApprove,the update entry for administrator is :"+afterApprove);
