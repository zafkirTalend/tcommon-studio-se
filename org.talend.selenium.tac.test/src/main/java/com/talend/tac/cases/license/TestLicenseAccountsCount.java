...
public class TestLicenseAccountsCount extends Login {
	...
	public void testDIUsersMoreOver() throws InterruptedException {
		...
		this.waitForElementPresent("//div[@class='x-grid3-cell-inner x-grid3-col-login' and text()='admin@company.com']", WAIT_TIME);
		String username = selenium.getText("//div[@class='x-grid3-cell-inner x-grid3-col-login' and text()='admin@company.com']");
		for(int i = 0; i < n-1; i++){
		selenium.refresh();
		Thread.sleep(3000);
		selenium.mouseDown("//div[@class='x-grid3-cell-inner x-grid3-col-login' and text()='"+username+"']");
		Thread.sleep(2000);
		...
		Thread.sleep(5000);
		if(i!=n-2){
		Assert.assertTrue(selenium.isTextPresent(username), "user duplicated failed!");
		}
		else{
			Assert.assertTrue(selenium.isTextPresent("Save failed: You are using "+(n-1)+" DI users, but your license allows only "+(n-1)+", please contact your talend account manager -- For more information see your log file"), "test failed!");
			Assert.assertFalse(selenium.isTextPresent(username), "user duplicated failed!");
		}
		}
		cleanAllExceptAdmin();
	}
	...
	@Test
	public void testDIUsersMoreOverUnactiveReactive() throws InterruptedException{
		cleanAllExceptAdmin();
		int n = getDIcounts();
		this.waitForElementPresent("idMenuUserElement", WAIT_TIME);
		selenium.setSpeed(MIN_SPEED);
		...
		for(int j = 0; j < n-1; j++){
			...
			selenium.mouseDown("//div[@class='x-grid3-cell-inner x-grid3-col-login' and text()='"+usernameReuse+"']");
			Thread.sleep(2000);
			...
			if(j==n-2){//the message needs to be make sure whether is right
//				Assert.assertTrue(selenium.isTextPresent("Save failed: You are using "+(n-1)+" DI users, but your license allows only "+(n-1)+", please contact your talend account manager -- For more information see your log file"), "test failed!");
	...
	public void testDQUsersMoreOver() throws InterruptedException{
		...
		Assert.assertTrue(selenium.isTextPresent(username), "user duplicated failed!");
			...
			Assert.assertTrue(selenium.isTextPresent("Save failed: You are using "+(n-1)+" DQ users, but your license allows only "+(n-1)+", please contact your talend account manager -- For more information see your log file"), "test failed!");
			Assert.assertFalse(selenium.isTextPresent(username), "user duplicated failed!");
		}
		selenium.setSpeed(MIN_SPEED);
		}
	...
	@Test
	public void testDQUsersMoreOverUnactiveReactive() throws InterruptedException{
		cleanAllExceptAdmin();
		int n = getDQcounts();
		this.waitForElementPresent("idMenuUserElement", WAIT_TIME);
		selenium.setSpeed(MIN_SPEED);
...
//				Assert.assertTrue(selenium.isTextPresent("Save failed: You are using "+(n-1)+" DQ users, but your license allows only "+(n-1)+", please contact your talend account manager -- For more information see your log file"), "test failed!");
	...
	public int getDIcounts(){
		...
		this.waitForElementPresent("//span[@class='x-panel-header-text' and text()='License Parameters']", WAIT_TIME);
        try {
			Thread.sleep(5000);
		...
		String allowedDI = selenium.getText("//div[@class='x-progress-text x-progress-text-back']//div[text()='Defined Data Integration users 1 / 5']");
	...
	public int getDQcounts(){
		...
		String allowedDI = selenium.getText("//div[@class='x-progress-text x-progress-text-back']//div[text()='Defined Data Quality users 0 / 5']");
        String num = allowedDI.split("/")[1];
        num = num.substring(num.indexOf(" ")+1, num.length());
        return ((int)Math.ceil((Integer.parseInt(num))*(moreOver+1.0)));
	}
