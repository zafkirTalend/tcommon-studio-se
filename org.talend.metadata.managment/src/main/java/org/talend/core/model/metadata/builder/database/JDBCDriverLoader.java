...
public class JDBCDriverLoader {
	 ...
	 * @return
	 */
	public List getConnection(String[] jarPath, String driverClassName,
			String url, String username, String password, String dbType,
			String dbVersion, String additionalParams) throws Exception {
		// qli modified to fix the bug 7656.
		List list = new ArrayList();
		HotClassLoader loader;
		boolean flog = EDatabaseVersion4Drivers.containTypeAndVersion(dbType,
				dbVersion);
		if (flog) {
			...
			for (int i = 0; i < jarPath.length; i++) {
				// bug 17800 fixed: fix a problem of jdbc drivers used in the
				// wizard.
		...
		try {
			...
			if (additionalParams != null && !"".equals(additionalParams)
				...
				info.load(new java.io.ByteArrayInputStream(additionalParams
						.getBytes()));
				connection = wapperDriver.connect(url, info);

			} else {
				if (dbType.equals(EDatabaseTypeName.HSQLDB_IN_PROGRESS
						.getDisplayName())
				...
				if (dbType.equals(EDatabaseTypeName.MSSQL.getDisplayName())) {
					connection = ConnectionUtils.createConnection(url,
							(Driver) (driver.newInstance()), info);
				} else {
					connection = wapperDriver.connect(url, info);
				}
			}
	...
	// Connection conn;
	// try {
	// conn = loader.getConnection(String[] jarPath, driverClassName, url,
	// username, password);
	// } catch (Throwable e) {
	// e.printStackTrace();
	// }
