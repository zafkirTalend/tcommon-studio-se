...
public class I18NChecker {
    ...
    private void processMapper(ProjectMapper mapper) throws Exception {
        ...
        File srcProperties = new File(project, "src/main/java/messages.properties");
        File desProperties = new File(nlProject, "src/main/java/messages_en.properties");

        File[] otherProperties = new File[] { new File(nlProject, "src/main/java/messages_zh.properties") };
...
//        merge(desProperties, otherProperties);
    ...
    private void merge(File enProperties, File otherProperties) throws Exception {
        ...
        otherPro.store(new FileOutputStream(otherProperties), "Generated by I18NChecker");
    }
    ...
    private void listProject() {
        ...
        for (int i = 0; i < projects.length; i++) {
            ...
            for (int j = 0; j < nlProjects.length; j++) {
                if (nlProjects[j].getName().equals(project.getName() + ".nl")) {
    ...
    FilenameFilter nlProjectFilter = new FilenameFilter() {
        ...
        public boolean accept(File dir, String name) {
            return name.startsWith("org.talend.") && name.endsWith(".nl");
        }
    };
    ...
    class ProjectMapper {
        ...
        public String toString() {
            return project + "-------" + mappingNLProject;
        }
    }
