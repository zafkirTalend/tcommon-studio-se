...
public class QueryUtil {
	...
	public static boolean needFormatSQL(String dbType) {
		if (dbType != null) {
			EDatabaseTypeName type = EDatabaseTypeName
					.getTypeFromDbType(dbType);
			if (type != null) {
				switch (type) {
				case NETEZZA:
				case ORACLE_OCI:
	...
	public static String generateNewQuery(IElement node,
		...
		if (generator != null) {
			generator.setParameters(node, repositoryMetadata, schema,
					realTableName);
			return generator.generateQuery();
		}
		return generateNewQueryDelegate(node, repositoryMetadata, dbType,
				schema, realTableName);
	}

	public static String generateNewQuery(IElement node,
			IMetadataTable repositoryMetadata, boolean isJdbc, String dbType,
			String schema, String realTableName) {
		IQueryGenerator generator = GenerateQueryFactory.getGenerator(dbType);
		if (generator != null) {
			generator.setParameters(node, repositoryMetadata, schema,
					realTableName, isJdbc);
			return generator.generateQuery();
		}
		return generateNewQueryDelegate(node, repositoryMetadata, dbType,
				schema, realTableName);
	...
	public static String generateNewQuery(IElement node,
		...
		return generateNewQueryDelegate(node, repositoryMetadata, dbType,
				schema, realTableName, standardSyntax);
	}

	public static String generateNewQueryDelegate(IElement node,
			IMetadataTable repositoryMetadata, String dbType, String schema,
			String realTableName) {
	 ...
	 * @return
	 */
	public static String generateNewQuery(
			final IMetadataTable repositoryMetadata, final String dbType,
			final String tableName, final boolean standardSyntax) {
		...
		for (int i = 0; i < metaDataColumnList.size(); i++) {
			...
			String columnName = TalendTextUtils
					.addQuotesWithSpaceFieldForSQLString(
							metaDataColumn.getOriginalDbColumnName(), dbType,
							true);
			if (!isContainDeclareString(columnName,
					TalendTextUtils.getStringDeclare())) {
				columnName = TalendTextUtils.declareString(columnName);
			}
			String columnStr = columnName;

			if (i != index - 1) {
				columnStr = checkAndConcatString(columnStr,
						TalendTextUtils.declareString("," + SPACE)); //$NON-NLS-1$
			}
		...
		if (isContextQuery) { // new line
			String end = ENTER
					+ CON
					+ checkAndConcatString(
							TalendTextUtils.declareString(" FROM "), TalendTextUtils //$NON-NLS-1$
									.addQuotesWithSpaceFieldForSQLString(
											tableName, dbType, !isContextQuery));
			...
			String end = checkAndConcatString(
					TalendTextUtils.declareString(" FROM "), TalendTextUtils //$NON-NLS-1$
							.addQuotesWithSpaceFieldForSQLString(tableName,
									dbType, !isContextQuery));
			end = replaceTheSchemaString(end);

			query = checkAndConcatString(
					checkAndConcatString(query, columnsQuery), end);
		}
	...
	public static String generateNewQuery(
		...
		for (int i = 0; i < metaDataColumnList.size(); i++) {
			IMetadataColumn metaDataColumn = metaDataColumnList.get(i);
			String columnName = quoteStringValue(
					metaDataColumn.getOriginalDbColumnName(), dbType);
			...
			if (dbType != null
					&& (dbType.equals(EDatabaseTypeName.MSSQL.getDisplayName()) || dbType
							.equals(EDatabaseTypeName.JAVADB_EMBEDED
									.getDisplayName()))) {
			...
			columnStr = checkAndConcatString(
					TalendTextUtils.declareString("."), columnStr); //$NON-NLS-1$
			columnStr = checkAndConcatString(tableNameWithQuoteIfNeed,
					columnStr);
		...
		if (isContextQuery) { // new line
			String end = ENTER
					+ CON
					+ checkAndConcatString(
							TalendTextUtils.declareString(" FROM "), tableNameWithQuoteIfNeed); //$NON-NLS-1$

			end = replaceTheSchemaString(end);

			query = checkAndConcatString(query, columnsQuery) + end;
		} else if (dbType != null
				&& dbType.equals(EDatabaseTypeName.INFORMIX.getDisplayName())) { // hywang
																					// add
																					// for
			// bug0007563
			...
			if (!isCheck) { // hywang add isCheck for informix
						...
						TalendTextUtils.declareString(" FROM "), declareString + realTableName[0] //$NON-NLS-1$
								+ declareString);
			} else {
				end = checkAndConcatString(
			...
			query = checkAndConcatString(
				...
				&& dbType.equals(EDatabaseTypeName.MSSQL.getDisplayName())) {
			String declareString = TalendTextUtils.getStringDeclare();
			String end = checkAndConcatString(
	...
	public static String getTableName(IElement node,
			...
			case JAVA:
				if (dbTableName.contains(TalendTextUtils.QUOTATION_MARK)) {
					if (dbTableName.startsWith(TalendTextUtils.QUOTATION_MARK)
						...
						currentTableName = dbTableName.substring(1,
					...
					if (null != currentTableName
							&& !("".equals(currentTableName))) { //$NON-NLS-1$
						flag = true;
					}
				}
				break;
		...
		if (schema != null && schema.length() > 0) {
			...
			if (isContext(currentTableName)) {
				currentTableName = quoteVariableRefrence(currentTableName,
						dbType);
			} else {
				currentTableName = quoteStringValue(currentTableName, dbType);
			}
	...
	private static String getSchemaName(String schema, String dbType,
			String currentTableName) {
		String prefix;
		String suffix;
		EDatabaseTypeName typeFromDbType = EDatabaseTypeName
				.getTypeFromDbType(dbType);
		if (typeFromDbType.isNeedSchema()) {
			// wzhang modified to fix bug 7879. value of oracle schema can't
			// attach quotation marks.
			boolean isOracle = (typeFromDbType == EDatabaseTypeName.ORACLEFORSID
					|| EDatabaseTypeName.ORACLESN == typeFromDbType || EDatabaseTypeName.ORACLE_OCI == typeFromDbType);
			if (isContext(schema)) {
				...
				if (isContext(currentTableName)) {
					prefix = checkAndConcatString(schemaStr,
							TalendTextUtils.declareString(".")); //$NON-NLS-1$
					suffix = quoteVariableRefrence(currentTableName, dbType);
				} else {
					prefix = schemaStr;
					suffix = checkAndConcatString(
							TalendTextUtils.declareString("."), quoteStringValue(currentTableName, dbType)); //$NON-NLS-1$
				}
			} else {
				String schemaStr = quoteStringValue(schema, dbType);
				if (isOracle) {
					boolean isCheck = CorePlugin.getDefault()
							.getPreferenceStore()
							.getBoolean(ITalendCorePrefConstants.SQL_ADD_QUOTE);
					if (isCheck) {
	...
	private static String quoteVariableRefrence(String ref, String dbType) {
		return TalendTextUtils.addQuotesWithSpaceFieldForSQLStringForce(ref,
				dbType, false);
	}

	private static String quoteStringValue(String value, String dbType) {
		return TalendTextUtils.addQuotesWithSpaceFieldForSQLStringForce(
				TalendTextUtils.declareString(value), dbType, true);
	}
	...
	private static String getDbName(Element node) {
		...
		if (node != null) { // for job settings extra.(feature 2710)
			...
			if (param != null && param.isShow(node.getElementParameters())) {
				dbName = TalendTextUtils
						.removeQuotes((String) param.getValue());
			}
		}
		return dbName;
	}
	...
	private static String checkAndConcatString(String str1, String str2) {
		...
		if (isContainDeclareString(str1, declareString)
				&& isContainDeclareString(str2, declareString)) {
	...
	private static boolean isContainDeclareString(String str,
	...
	public static String checkAndAddQuotes(String query) {
		...
		query = query.trim();
		// modified by hyWang
		if (!query.startsWith(TalendTextUtils.getQuoteChar())
				|| !TalendTextUtils.isCommonString(query)) { // perhaps,
			// need
			// improve
	 ...
	 * 
	 * if the query is common string and start with quote, will remove the
	 * quotes.
	 */
	public static String checkAndRemoveQuotes(String query) {
		...
		query = query.trim();
		if (query.startsWith(TalendTextUtils.getQuoteChar())
				&& TalendTextUtils.isCommonString(query)) {
