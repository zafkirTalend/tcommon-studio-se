...
public class MetadataTool {
	...
	public static List<ColumnNameChanged> getColumnNameChanged(
			IMetadataTable oldTable, IMetadataTable newTable) {
		...
		for (int i = 0; i < oldTable.getListColumns().size(); i++) {
			...
			IMetadataColumn clonedColumn = getColumn(newTable, originalColumn
					.getId());
			if (clonedColumn != null) {
				if (!originalColumn.getLabel().equals(clonedColumn.getLabel())) {
					columnNameChanged.add(new ColumnNameChanged(originalColumn
							.getLabel(), clonedColumn.getLabel()));
				}
			}
		}
	...
	public static List<ColumnNameChanged> getColumnNameChangedExt(
			INode changedNode, IMetadataTable oldTable, IMetadataTable newTable) {
		...
		for (int i = 0; i < oldTable.getListColumns().size(); i++) {
			...
			if (clonedColumn != null) {
				if (!originalColumn.getLabel().equals(clonedColumn.getLabel())) {
					columnNameChanged
							.add(new ColumnNameChangedExt(changedNode,
									originalColumn.getLabel(), clonedColumn
											.getLabel()));
				}
			}
		}
	...
	public static void copyTable(IMetadataTable source, IMetadataTable target,
		...
		for (IMetadataColumn column : source.getListColumns()) {
			...
			if (targetColumn == null) {
				...
				newTargetColumn
						.setReadOnly(target.isReadOnly()
								|| readOnlycolumns.contains(newTargetColumn
										.getLabel()));
			} else {
	...
	public static IMetadataTable getMetadataFromRepository(
			String metaRepositoryId) {
		org.talend.core.model.metadata.builder.connection.Connection connection;
		...
		if (conn instanceof SAPConnection) {
			...
			if (functions != null && !functions.isEmpty()) {
				...
				for (int i = 0; i < functions.size(); i++) {
					tables.add(((SAPFunctionUnit) functions.get(i))
							.getMetadataTable());
				}
				return tables;
			}
	...
	public static org.talend.core.model.metadata.builder.connection.Connection getConnectionFromRepository(
		...
		try {
			...
			if (factory.getStatus(object) != ERepositoryStatus.DELETED) {
				return ((ConnectionItem) object.getProperty().getItem())
						.getConnection();
			}
	 ...
	 * @return
	 */
	public static List<ColumnNameChanged> getNewMetadataColumns(
			IMetadataTable oldTable, IMetadataTable newTable) {
	 ...
	 * @return
	 */
	public static List<ColumnNameChanged> getRemoveMetadataColumns(
			IMetadataTable oldTable, IMetadataTable newTable) {
		List<ColumnNameChanged> list = new ArrayList<ColumnNameChanged>();
		List<IMetadataColumn> newColumns = newTable.getListColumns();
		List<IMetadataColumn> oldColumns = oldTable.getListColumns();
	...
	public static void initilializeSchemaFromElementParameters(
			...
			if (param.getField().equals(EParameterFieldType.SCHEMA_TYPE)
				...
				if (param.getValue() instanceof IMetadataTable) {
					...
					for (int k = 0; k < table.getListColumns().size(); k++) {
						IMetadataColumn newColumn = table.getListColumns().get(
								k);
						IMetadataColumn oldColumn = metadataTable
								.getColumn(newColumn.getLabel());
						if (oldColumn != null) {
	...
	public static IElementParameter getMappingParameter(
			List<IElementParameter> elementParameters) {
		for (int i = 0; i < elementParameters.size(); i++) {
			IElementParameter param = elementParameters.get(i);
	...
	public static void copyTable(IMetadataTable source, MetadataType target) {
		...
		colTypes.clear();
		for (IMetadataColumn column : source.getListColumns()) {
			ColumnType createColumnType = TalendFileFactory.eINSTANCE
					.createColumnType();
			...
			if (!column.getLabel().equals(column.getOriginalDbColumnName())) {
				createColumnType.setOriginalDbColumnName(column
