...
public class ExtractMetaDataUtils {
	 ...
	 */
	public static String getStringMetaDataInfo(ResultSet columns,
			String infoType) {
	 ...
	 */
	public static boolean getBooleanMetaDataInfo(ResultSet columns,
			String infoType) {
	...
	public static String getDriverClassByDbType(String dbType) {
		...
		try {
			...
			hashTable
					.put(
							"Oracle with service name", "oracle.jdbc.driver.OracleDriver"); //$NON-NLS-1$ //$NON-NLS-2$
			hashTable.put("Generic ODBC", "sun.jdbc.odbc.JdbcOdbcDriver"); //$NON-NLS-1$ //$NON-NLS-2$
			hashTable
					.put(
							"Microsoft SQL Server (Odbc driver)", "sun.jdbc.odbc.JdbcOdbcDriver"); //$NON-NLS-1$ //$NON-NLS-2$
			...
			// //$NON-NLS-1$ //$NON-NLS-2$
			hashTable.put(
					"Microsoft SQL Server", "net.sourceforge.jtds.jdbc.Driver"); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 *            schemaBase
	 */
	public static void getConnection(String dbType, String url,
			String username, String pwd, String dataBase, String schemaBase) {
		try {
			Class.forName(getDriverClassByDbType(dbType)).newInstance();
			conn = DriverManager.getConnection(url, username, pwd);
			if (schemaBase != null && !schemaBase.equals("")) { //$NON-NLS-1$
				final boolean equals = EDatabaseTypeName.getTypeFromDbType(
						dbType).getProduct().equals(
						EDatabaseTypeName.ORACLEFORSID.getProduct());
				if (!ExtractMetaDataFromDataBase.checkSchemaConnection(
						schemaBase, conn, equals)) {
					schema = null;
				}
			} else {
				schema = null;
