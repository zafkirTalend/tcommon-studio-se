...
public final class ContextParameterUtils {
	 ...
	 * 
	 * @param parameter
	 *            Context parameter used in script.
	 * @param language
	...
	public static String getScriptCode(IContextParameter parameter,
		...
		final String string = JAVA_STARTWITH + parameter.getName()
				+ JAVA_ENDWITH;
		switch (language) {
		case PERL:
			...
			break;
		case JAVA:
			JavaType javaType = ContextParameterJavaTypeManager
					.getJavaTypeFromId(parameter.getType());
			String typeToGenerate = ContextParameterJavaTypeManager
					.getTypeToGenerate(parameter.getType(), true);
			if (javaType.isPrimitive()) {
				if (typeToGenerate.compareTo("String") == 0) {
					...
					code = typeToGenerate + ".parse" + typeToGenerate + "("
							+ string + ")";
				}
	...
	public static String parseScriptContextCode(String code,
	...
	public static Date getDate(String s) {
		try {
			final Date parse = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
					.parse(s);
			return parse;
		} catch (Exception e) {
	...
	public static String parseScriptContextCode(String code, IContext context) {
		...
		final ECodeLanguage language = ((RepositoryContext) CorePlugin
	...
	private static String getContextString(String code, ECodeLanguage language) {
		...
		case PERL:
			if (containOldContext(code)) {
				return code.substring(code.indexOf(PERL_STARTWITH)
						+ PERL_STARTWITH.length(), code.indexOf(PERL_ENDWITH));
			}
		case JAVA:
			if (containOldContext(code)) {
				return code.substring(code.indexOf(JAVA_STARTWITH)
						+ JAVA_STARTWITH.length(), code.indexOf(JAVA_ENDWITH));
	...
	private static boolean containOldContext(String code) {
		...
		switch (language) {
		case PERL:
			return (code.contains(PERL_STARTWITH) && code
					.contains(PERL_ENDWITH));
		case JAVA:
			return (code.contains(JAVA_STARTWITH.substring(0, JAVA_STARTWITH
					.length() - 1)) && code.contains(JAVA_ENDWITH));
		default:
			return false;
		}
	}
	 ...
	 * 
	 * @param name
	 *            Context parameter name tested.
	 * @param language
	 *            Language where the context parameter is used.
