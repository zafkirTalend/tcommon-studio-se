...
/**
 * Preference page for known servers. <br/>
 * 
 * $Id$
 * 
 */
public final class ServerPreferencePage extends ListPreferencePage {

    /**
     * Constructs a new ServerPreferencePage.
     */
    ...
    protected String[] getDataSaved() {
        ...
        return prefManipulator.readServers();
    }
    ...
    public boolean performOk() {
        PreferenceManipulator prefManipulator = new PreferenceManipulator(getPreferenceStore());
        prefManipulator.saveServers(getDataToSave());
    ...
    protected int[] getDeletableIndices(int[] indices) {
        ...
        String connectedServer = repositoryContext.getServer();
        int disallowedIndex = -1;
        for (int i = 0; disallowedIndex == -1 && i < indices.length; i++) {
            if (dataList.getItem(indices[i]).equals(connectedServer)) {
                disallowedIndex = i;
            }
        }
