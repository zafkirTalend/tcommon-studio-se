...
public abstract class ReconcilerViewer extends ProjectionViewer {
	...
	public ReconcilerViewer(Composite parent, IVerticalRuler verticalRuler,
			IOverviewRuler overviewRuler, boolean showAnnotationsOverview,
			int styles, IAnnotationAccess annotationAccess,
			ISharedTextColors sharedColors, boolean checkCode,
			IDocument document, final INode node) {
	...
	private void addDocumentListener(final IDocument document) {
		final ExecutionLimiter documentReconcilerLimiter = new ExecutionLimiter(
				500, true) {
			...
			Display.getDefault().asyncExec(new Runnable() {
				...
				public void run() {
					...
					if (getProjectionAnnotationModel() != null) {
						Annotation[] oldAnno = oldAnnotations.keySet().toArray(
								new Annotation[0]);
						getProjectionAnnotationModel().modifyAnnotations(
								oldAnno, annotations, null);
	...
	private boolean hasSnippetsModifications() {
		...
		FindReplaceDocumentAdapter frda = new FindReplaceDocumentAdapter(
				document);
		try {
			IRegion startRegion = frda.find(curOffset, "SNIPPET_START", true,
					false, false, false);
			while (startRegion != null && startRegion.getOffset() >= curOffset) {
				int startLine = document.getLineOfOffset(startRegion
						.getOffset());
				int startOffset = document.getLineOffset(startLine);
				curOffset = startOffset + document.getLineLength(startLine);
				IRegion endRegion = frda.find(startRegion.getOffset(),
						"SNIPPET_END", true, false, false, false);
				if (endRegion != null) {
					actualNbAnnotations++;
					int endLine = document.getLineOfOffset(endRegion
							.getOffset());
					int endOffset = document.getLineOffset(endLine);
					endOffset += document.getLineLength(endLine);
					curOffset = endOffset;
					boolean contains = false;
					String text = document.get(startOffset, endOffset
							- startOffset);
					for (ProjectionAnnotation annotation : oldAnnotations
							.keySet()) {
				...
				if (curOffset < document.getLength()) {
					startRegion = frda.find(curOffset, "SNIPPET_START", true,
							false, false, false);
				}
			}
		...
		try {
			...
			while (startRegion != null && startRegion.getOffset() >= curOffset) {
				...
				if (endRegion != null) {
					...
					annotations.put(annotation, new Position(startOffset,
	...
	private void addMenu() {
		...
		Image image = ImageProvider.getImage(sharedImages
		...
		image = ImageProvider.getImage(sharedImages
	...
	/**
	 * Example: configure(new
	 * TalendPerlSourceViewerConfiguration(PerlEditorPlugin
	...
	protected SourceViewerDecorationSupport getSourceViewerDecorationSupport() {
		if (fSourceViewerDecorationSupport == null) {
			fSourceViewerDecorationSupport = new SourceViewerDecorationSupport(
					this, fOverviewRuler, annotationAccess, sharedColors);
	...
	protected void initializeModel() {
		ProjectionSupport projectionSupport = new ProjectionSupport(this,
				annotationAccess, sharedColors);
		projectionSupport
		...
		projectionSupport
				.setHoverControlCreator(new IInformationControlCreator() {
					...
					public IInformationControl createInformationControl(
							Shell shell) {
						return new SourceViewerInformationControl(shell, false,
								SWT.LEFT_TO_RIGHT, EditorsUI
	...
	public IRegion getViewerRegion() {
		...
		return new Region(viewerStartRegion.getLength(), getDocument()
				.getLength()
				- viewerStartRegion.getLength() - viewerEndRegion.getLength());
	}
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.text.source.projection.ProjectionViewer#setVisibleRegion
	 * (int, int)
	 */
	@Override
	public void setVisibleRegion(int start, int length) {
		...
		if (getDocument().getLength() > start) {
			viewerEndRegion = new Region(start + 1 + length, getDocument()
					.getLength()
					- start - length);
		} else {
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.text.TextViewer#createTextWidget(org.eclipse.swt.widgets
	 * .Composite, int)
	 */
	@Override
	...
	protected void updateVisibleRegion() {
		if (this.getDocument() == null || this.viewerEndRegion == null
				|| this.getVisibleDocument() == null) {
			return;
		}
		...
		if (this.oldDocLength != newDocLength) {
			...
			final int newStart = newDocLength - newLength
					- this.viewerEndRegion.getLength();
