...
public class MetadataDialog extends Dialog {
	...
	public MetadataDialog(Shell parent, IMetadataTable inputMetaTable,
			INode inputNode, IMetadataTable outputMetaTable, INode outputNode,
		...
		if (inputNode != null) {
			...
			INodeConnector connector = inputNode
					.getConnectorFromName(inputMetaTable.getAttachedConnector());
			if (!connector.isBuiltIn()) {
				this.titleInput = inputNode.getLabel() + " (Input - "
						+ connector.getLinkName() + ")";
			}
		}
	...
	public MetadataDialog(Shell parent, IMetadataTable outputMetaTable,
	...
	public static void initializeMetadataTableView(
			MetadataTableEditorView metaView, INode node,
		...
		if (node.getComponent().getFamily().startsWith(DATABASE_LABEL)
			...
			for (IElementParameter currentParam : node.getElementParameters()) {
				if (currentParam.getField().equals(
						EParameterFieldType.MAPPING_TYPE)
			...
			IElementParameter schemaParam = node
					.getElementParameter("SCHEMA_TYPE");
			if (!hasMappingType && schemaParam != null) { // if there is no
				...
				if (schemaType.equals("REPOSITORY")) {
					// repository mode
					String metaRepositoryName = (String) node
							.getElementParameter("REPOSITORY_SCHEMA_TYPE")
							.getValue();
					Connection connection = MetadataTool
							.getConnectionFromRepository(metaRepositoryName);
					if (connection instanceof DatabaseConnection) {
						hasRepositoryDbSchema = true;

						for (IMetadataColumn column : metadataTable
								.getListColumns()) {
							if ((column.getType() == "")
									|| (column.getType() == null)) {
						...
						String componentProduct = EDatabaseTypeName
								.getTypeFromDbType(componentDbType)
								.getProduct();
						...
						String connectionProduct = ((DatabaseConnection) connection)
								.getProductId();
						if (!componentProduct.equals(connectionProduct)) {
							...
							metaView
									.setCurrentDbms(((DatabaseConnection) connection)
											.getDbmsId());
						}
		...
		metaView.setShowTalendTypeColumn(!node.getComponent().getFamily()
				.startsWith(ELT_LABEL));
		// hide the pattern for ELT components
		metaView.setShowPatternColumn(!node.getComponent().getFamily()
				.startsWith(ELT_LABEL));
	}
	...
	protected Control createDialogArea(final Composite parent) {
		...
		if (inputMetaTable == null) {
			...
			metadataTableEditor = new MetadataTableEditor(outputMetaTable,
					titleOutput);
			outputMetaView = new MetadataTableEditorView(composite, SWT.NONE,
					metadataTableEditor, outputReadOnly, true, true, false);

			initializeMetadataTableView(outputMetaView, outputNode,
					outputMetaTable);
			outputMetaView.initGraphicComponents();
			outputMetaView.getExtendedTableViewer().setCommandStack(
					commandStack);
		} else {
			compositesSachForm = new ThreeCompositesSashForm(composite,
					SWT.NONE);
			...
			metadataTableEditor = new MetadataTableEditor(inputMetaTable,
					titleInput); //$NON-NLS-1$
			inputMetaView = new MetadataTableEditorView(compositesSachForm
					.getLeftComposite(), SWT.NONE, metadataTableEditor,
					inputReadOnly, true, true, false);
			initializeMetadataTableView(inputMetaView, inputNode,
					inputMetaTable);
			inputMetaView.initGraphicComponents();
			inputMetaView.getExtendedTableViewer()
					.setCommandStack(commandStack);

			inputMetaView.setGridDataSize(size.x / 2 - 50, size.y - 150);

			Label label1 = new Label(compositesSachForm.getMidComposite(),
					SWT.NONE);
			GridDataFactory.swtDefaults().hint(42, 18).applyTo(label1);
			Composite buttonComposite = new Composite(compositesSachForm
					.getMidComposite(), SWT.BORDER);
			Label label2 = new Label(compositesSachForm.getMidComposite(),
					SWT.NONE);
			...
			Composite buttonComposite2 = new Composite(buttonComposite,
					SWT.NONE);
			...
			copyToOutput.setToolTipText(Messages
					.getString("MetadataDialog.CopyToOutput.ToolTopText")); //$NON-NLS-1$
			GridDataFactory.swtDefaults().align(SWT.CENTER, SWT.CENTER)
					.applyTo(copyToOutput);
			copyToOutput.addListener(SWT.Selection, new Listener() {
				...
				public void handleEvent(Event event) {
					...
					messageBox.setText(Messages
							.getString("MetadataDialog.SchemaModification")); //$NON-NLS-1$
					messageBox.setMessage(Messages
							.getString("MetadataDialog.Message")); //$NON-NLS-1$
					if (messageBox.open() == SWT.OK) {
						MetadataTool.copyTable(getInputMetaData(),
								getOutputMetaData());
						outputMetaView.getTableViewerCreator().getTableViewer()
								.refresh();
					}
				}
			});
			...
			copyToInput.setToolTipText(Messages
					.getString("MetadataDialog.CopyToInput")); //$NON-NLS-1$
			...
			copyToInput.addListener(SWT.Selection, new Listener() {
				...
				public void handleEvent(Event event) {
					...
					messageBox.setText(Messages
							.getString("MetadataDialog.SchemaModification")); //$NON-NLS-1$
					messageBox.setMessage(Messages
							.getString("MetadataDialog.TransferMessage")); //$NON-NLS-1$
					if (messageBox.open() == SWT.OK) {
						MetadataTool.copyTable(getOutputMetaData(),
								getInputMetaData());
						inputMetaView.getTableViewerCreator().getTableViewer()
								.refresh();
					}
				}
			});
			...
			outputMetaView = new MetadataTableEditorView(compositesSachForm
					.getRightComposite(), SWT.NONE,
					new MetadataTableEditor(outputMetaTable, titleOutput
			...
			CustomTableManager.addCustomManagementToTable(inputMetaView,
					inputReadOnly);
			CustomTableManager.addCustomManagementToToolBar(inputMetaView,
					inputMetaTable, inputReadOnly, outputMetaView,
					outputMetaTable, outputNode.getComponent()
							.isSchemaAutoPropagated());
		}
		CustomTableManager.addCustomManagementToTable(outputMetaView,
				outputReadOnly);
		CustomTableManager.addCustomManagementToToolBar(outputMetaView,
				outputMetaTable, outputReadOnly, inputMetaView, inputMetaTable,
				false);
		metadataTableEditor
				.addModifiedBeanListener(new IModifiedBeanListener<IMetadataColumn>() {

					public void handleEvent(
							ModifiedBeanEvent<IMetadataColumn> event) {
						if ((inputMetaTable != null)
								&& outputMetaTable.isReadOnly()
								&& outputNode.getComponent()
										.isSchemaAutoPropagated()) {
							MetadataTool.copyTable(inputMetaTable,
									outputMetaTable);
							outputMetaView.getTableViewerCreator()
									.getTableViewer().refresh();
						}
						if (AbstractMetadataTableEditorView.ID_COLUMN_NAME
								.equals(event.column.getId())) {
							IMetadataColumn modifiedObject = (IMetadataColumn) event.bean;
							if (modifiedObject != null) {
								String originalLabel = changedNameColumns
										.get(modifiedObject);
								if (originalLabel == null) {
									changedNameColumns.put(modifiedObject,
