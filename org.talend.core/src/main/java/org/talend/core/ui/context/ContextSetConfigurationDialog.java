 ...
 * 
 */
public class ContextSetConfigurationDialog extends
		ObjectSelectionDialog<IContext> {
	...
	/**
	 * DOC bqian ContextSetConfigurationDialog class global comment. Detailled
	 * comment <br/>
	 * 
	 */
	...
	public void createElement() {
		InputDialog inputDial = new InputDialog(getShell(), Messages
				.getString("ContextProcessSection.6"), //$NON-NLS-1$
	...
	private boolean validateContextName(String name) {
		if (name.equals("")
				|| !name.matches(RepositoryConstants.CODE_ITEM_PATTERN)) {
	...
	private void createContext(final String name) {
		...
		for (int i = 0; i < context.getContextParameterList().size(); i++) {
			param = (JobContextParameter) context.getContextParameterList()
					.get(i).clone();
	...
	protected void editSelectedElement() {
		IContext selectedContext = (IContext) (getSelection())
				.getFirstElement();
		String contextName = selectedContext.getName();
		InputDialog inputDial = new InputDialog(
				getShell(),
				Messages.getString("ContextProcessSection.12"), //$NON-NLS-1$
		...
		if (manager.getProcess().getLastRunContext().sameAs(selectedContext)) {
	...
	protected void updateButtonAvailability() {
		...
		if (selectDefaultContext) {
			...
			String contextNname = manager.getContextManager()
					.getDefaultContext().getName();
			setDisplayMessage(Messages.getString(
					"ContextProcessSection.RemoveInformation", contextNname));
		} else {
