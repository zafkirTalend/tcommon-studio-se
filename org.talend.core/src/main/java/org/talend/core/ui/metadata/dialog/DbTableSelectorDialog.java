...
public class DbTableSelectorDialog extends Dialog {
	...
	private static Image dbImage = CorePlugin.getImageDescriptor(
			"/icons/database/connection.png").createImage();

	private static Image columnImage = CorePlugin.getImageDescriptor(
			"/icons/database/columns.gif").createImage();

	private static Image tableImage = CorePlugin.getImageDescriptor(
			"/icons/database/table.gif").createImage();
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setLocation(getParentShell().getLocation().x
				+ getParentShell().getSize().x / 2, getParentShell()
				.getLocation().y
	...
	protected Control createDialogArea(Composite parent) {
		...
		viewer = new TreeViewer(container, SWT.SINGLE | SWT.H_SCROLL
			...
			public void selectionChanged(SelectionChangedEvent event) {
				if (event.getSelection() instanceof StructuredSelection) {
					...
					if (firstElement instanceof DbTableSelectorObject) {
						...
						if (dbTableSelectorObject.getType().compareTo(
								ObjectType.TABLE) == 0) {
	...
	/**
	 * qzhang DbTableSelectorDialog class global comment. Detailled comment
	 * <br/>
	 * 
	 */
	private class ListProvider extends LabelProvider implements
		...
		public Object[] getElements(Object inputElement) {
			if (inputElement instanceof DbTableSelectorObject) {
				final List<DbTableSelectorObject> children = ((DbTableSelectorObject) inputElement)
						.getChildren();
				if (children != null) {
