...
public final class UpdateRepositoryUtils {
				...
				if (queryConn != null) {
					...
					if (queries != null) {
						for (Query query : (List<Query>) queries) {
							Object object = query.getProperties()
									.get("deleted"); //$NON-NLS-1$
							if (object == null
									|| (object != null && !object
	...
	public static Query getQueryById(Map<String, ConnectionItem> itemsMap,
	...
	public static MetadataTable getTableById(
			Map<String, ConnectionItem> itemsMap, final String tableId) {
	 ...
	 * @return
	 */
	public static ConnectionItem getConnectionItemByChildId(
			Map<String, ConnectionItem> itemsMap, final String childId) {
	...
	public static ConnectionItem getConnectionItemByChildId(final String childId) {
		final IProxyRepositoryFactory proxyRepositoryFactory = CorePlugin
				.getDefault().getProxyRepositoryFactory();
		try {
			List<ConnectionItem> metadataConnectionsItem = proxyRepositoryFactory
	...
	public static ConnectionItem getConnectionItemByItemId(String itemId,
		...
		final IProxyRepositoryFactory proxyRepositoryFactory = CorePlugin
				.getDefault().getProxyRepositoryFactory();
		try {
			final IRepositoryObject lastVersion = proxyRepositoryFactory
					.getLastVersion(itemId);
			if (lastVersion != null) {
				if (!deleted
						&& proxyRepositoryFactory.getStatus(lastVersion) == ERepositoryStatus.DELETED) {
					return null;
				}
	...
	public static List<Query> getQueriesFromItem(Item item) {
		...
		if (item instanceof ConnectionItem) {
			final Connection connection = ((ConnectionItem) item)
					.getConnection();
			if (connection != null) {
				final QueriesConnection queryConn = connection.getQueries();
				if (queryConn != null) {
	 ...
	 */
	public static List<MetadataTable> getMetadataTablesFromItemId(
			final String itemId) {
		ConnectionItem item = getConnectionItemByItemId(itemId);
		if (item != null) {
			final Connection connection = item.getConnection();
	...
	public static IRepositoryObject getRepositoryObjectById(final String id) {
		...
		try {
			...
			if (lastVersion != null
					&& factory.getStatus(lastVersion) != ERepositoryStatus.DELETED) {
				return lastVersion;
			}
		} catch (PersistenceException e) {
			//
	...
	public static String getRepositorySourceName(Item item) {
		...
		if (item != null) {
			...
			if (item instanceof ConnectionItem) {
				...
				if (connection instanceof DatabaseConnection) {
					String currentDbType = (String) RepositoryToComponentProperty
							.getValue(connection, UpdatesConstants.TYPE);
			...
			source = source + UpdatesConstants.COLON
		...
		if (connection != null) {
			...
			if (tables != null) {
				for (MetadataTable table : (List<MetadataTable>) tables) {
					Object object = table.getProperties().get("deleted"); //$NON-NLS-1$
					if (object == null
							|| (object != null && !object.equals(Boolean.TRUE
									.toString()))) {
