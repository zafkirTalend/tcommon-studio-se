 ...
 * 
 * @param <D1>
 *            the data item of extremety 1
 * @param <D2>
 *            the data item of extremety 2
 */
public class TreeToTablesLinker<D1, D2> extends BgDrawableComposite implements
	 ...
	 */
	public void init(Tree tree, Table[] tables,
			IBackgroundRefresher backgroundRefresher) {
	...
	protected IStyleLink getDefaultStyleLink() {
		if (defaultStyleLink == null) {
			...
			styleLink.setForegroundColor(display
	...
	protected Comparator<LinkDescriptor<TreeItem, D1, Table, D2>> getDrawingLinksComparator() {
		if (this.drawingLinksComparator == null) {
			this.drawingLinksComparator = new Comparator<LinkDescriptor<TreeItem, D1, Table, D2>>() {
				...
				public int compare(
						LinkDescriptor<TreeItem, D1, Table, D2> link1,
	...
	public void drawBackground(GC gc) {
		...
		List<LinkDescriptor<TreeItem, D1, Table, D2>> links = linksManager
				.getLinks();
		int lstSize = links.size();

		int xStartBezierLink = findXRightStartBezierLink(tree.getItems(), 0);

		Point treeToCommonPoint = display.map(tree, getBgDrawableComposite(),
				new Point(0, 0));
		...
		Rectangle treeBounds = tree.getBounds();
		 ...
		 if(WindowSystem.isGTK()) {
			 ...
			 gc.fillRectangle(treeToCommonPoint.x, treeToCommonPoint.y,
					 treeBounds.width - tree.getBorderWidth(), treeBounds.height
					 - tree.getBorderWidth());
		...
		for (int i = 0; i < lstSize; i++) {
			...
			Point tableToCommonPoint = display.map(table,
					getBgDrawableComposite(), new Point(0, 0));
			...
			TreeItem treeItem = TreeUtils.getTreeItem(tree, (Object) extremity1
					.getDataItem());

			TreeItem firstExpandedAscTreeItem = TreeUtils
					.findFirstVisibleItemAscFrom(treeItem);

			Rectangle treeItemBounds = firstExpandedAscTreeItem.getBounds();

			int yStraight = treeToCommonPoint.y + treeItemHeight / 2
					+ treeItemBounds.y;
			Point pointStartStraight = new Point(treeToCommonPoint.x
					+ treeItemBounds.x + treeItemBounds.width, yStraight);
			Point pointEndStraight = new Point(treeToCommonPoint.x
					+ xStartBezierLink, yStraight);

			Rectangle tableItemBounds = TableUtils.getTableItem(table,
					(Object) extremity2.getDataItem()).getBounds();
			Rectangle tableBounds = table.getBounds();

			Point pointEndCentralCurve = backgroundRefresher
					.convertPointToCommonParentOrigin(new Point(
							tableItemBounds.x - 2, tableItemBounds.y
									+ table.getItemHeight() / 2
									+ table.getBorderWidth()), table);

			boolean lineStyleDot = false;

			Point point = display.map(tree, getBgDrawableComposite(),
					new Point(0, 0));
			...
			if (pointEndCentralCurve.y > tableToCommonPoint.y
					+ tableBounds.height) {
				pointEndCentralCurve.y = tableToCommonPoint.y
						+ tableBounds.height - 2 * table.getBorderWidth();
				lineStyleDot = true;
			}
			...
			gc.drawLine(pointStartStraight.x + offset.x, pointStartStraight.y
					+ offset.y, pointEndStraight.x + offset.x,
	...
	private int findXRightStartBezierLink(TreeItem[] items, int maxWidth) {
		for (int i = 0; i < items.length; i++) {
			...
			if (!item.isDisposed()) {
				...
				if (item.getExpanded()) {
					maxWidth = findXRightStartBezierLink(item.getItems(),
							maxWidth);
				}
			}
		}
	...
	public void updateLinksStyleAndControlsSelection(Control currentControl) {
		...
		if (currentControl instanceof Table) {
			...
			throw new IllegalArgumentException(
					"This type of currentControl is unsupported");
		}
		...
		for (LinkDescriptor<TreeItem, D1, Table, D2> link : links) {
			...
			boolean currentItemIsSelected = selectedItems.contains(extremity
					.getDataItem());
	 ...
	 * 
	 * @param selectedStyleLink
	 *            the selectedStyleLink to set
	 */
	 ...
	 * 
	 * @param unselectedStyleLink
	 *            the unselectedStyleLink to set
	 */
	 ...
	 * 
	 * @param backgroundRefresher
	 *            the backgroundRefresher to set
	 */
