...
/**
 * A dnd utility class to help performing drag and drop. This code will be in SWTbot in future but need to be polished
 * before.
 * 
 * @author mchauvin
 */
public class DndUtil {
    ...
    /**
     * Number of pixels traversed before a drag starts. Ms : 10 OSX : 10(1.3.1), 5(1.4.1) Linux/X11: delay+16
     */
    private static final int DRAG_THRESHOLD = "gtk".equals(SWT.getPlatform()) ? 16 : 10;

    /**
     * delay before moving after mouse is pressed. could be used for delay after moving before mouse is released
     */
     ...
     * 
     * @param source the source widget to drag
    ...
    /**
     * Performs a drag and drop operation from this widget to the given target at the given location from target origin.
     * The drag start location will be chosen depending on this widget's default implementation.
     * 
     * @param source the source widget to drag
     * @param target To perform the drop on
     * @param locationOnTarget The target locations, from target origin, where the DND shall finish.
     * @see #dragAndDrop(Point)
     */
    public void dragAndDrop(final AbstractSWTBot<? extends Widget> source, final AbstractSWTBot<? extends Widget> target,
     ...
     */
    public void dragAndDrop(final AbstractSWTBot<? extends Widget> source, final AbstractSWTBot<? extends Widget> target,
            final org.eclipse.draw2d.geometry.Point locationOnTarget) {
    ...
    private static <T extends Widget> Rectangle absoluteLocation(final AbstractSWTBot<T> item) {
        ...
        } else if (item instanceof SWTBotTable) {
            bot = new SWTBotTableForDnd(((SWTBotTable) item).widget);
        } else {
            bot = item;
        }
        Object result = null;
    ...
    private static class SWTBotTableItemForDnd extends SWTBotTableItem {
        ...
        public SWTBotTableItemForDnd(TableItem tableItem) throws WidgetNotFoundException {
            super(tableItem);
        }
     ...
     * @author fzhong
     */
    private static class SWTBotTableForDnd extends SWTBotTable {

        public SWTBotTableForDnd(Table table) throws WidgetNotFoundException {
            super(table);
        }
