...
public class UseRegexFileTest extends TalendSwtBotForTos {
    ...
    public void createJobAndMetadata() throws IOException, URISyntaxException {
        ...
        metadataNode = Utilities.getTalendItemNode(tree, Utilities.TalendItemType.FILE_REGEX);
    ...
    public void useMetadataInJob() throws IOException, URISyntaxException {
        TalendRegexFileItem rfItem = new TalendRegexFileItem();
        rfItem.setItem(fileItem);
        rfItem.setFilePath(System.getProperty("fileRegex.filepath"));
        rfItem.setComponentType("tFileInputRegex");
        MetadataHelper.output2Console(jobEditor, rfItem);

        String result = gefBot.styledText().getText();
        MetadataHelper.assertResult(result, rfItem);
    }
