...
public class DeleteLdapTest extends TalendSwtBotForTos {
    ...
    public void deleteLdap() {
        tree.expandNode("Metadata", "LDAP").getNode(LDAPNAME + " 0.1").contextMenu("Delete").click();

        SWTBotTreeItem newLdapItem = null;
        try {
            newLdapItem = tree.expandNode("Recycle bin").select(LDAPNAME + " 0.1" + " ()");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            Assert.assertNotNull("ldap item is not deleted to recycle bin", newLdapItem);
        }
    }
