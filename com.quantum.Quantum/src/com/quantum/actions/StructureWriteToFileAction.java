...
public class StructureWriteToFileAction extends SelectionListenerAction {
	private final IViewPart view;
	
	public StructureWriteToFileAction(IViewPart view) {
		super(Messages.getString(StructureWriteToFileAction.class, "text"));
		this.view = view;
		setImageDescriptor(
				ImageStore.getImageDescriptor(ImageStore.EXPORT));
	}
	...
	public void run() {
        ...
        if ( selection instanceof DbObjectNode 
			...
			FileOutputStream out = ViewHelper.askSaveFile("exportStructure", view.getSite().getShell(), 
					new String[]{Messages.getString("filedialog.qusFiles.filter"), Messages.getString("filedialog.allfiles.filter")},
					new String[]{Messages.getString("filedialog.qusFiles"),	Messages.getString("filedialog.allfiles")
					});
			if (out == null)
				return;
			...
			MessageDialog.openInformation(
				      ...
				      "Your selection cannot be currently exported. Try selecting a table, view, schema, quick list...");
			return;
        }
