// $ANTLR : "SQL92SelectParser.g" -> "Sql92SelectParser.java"$
...
public class Sql92SelectParser extends antlr.LLkParser       implements Sql92SelectParserTokenTypes
 {
	...
	public static final String STATE_WHERE = "Where";
	public static final String STATE_CONDITION = "Condition";
	public static final String STATE_GROUP = "Group By";
	public static final String STATE_ORDER = "Order By";
	public static final String STATE_JOIN_CONDITION = "Join Condition";

protected Sql92SelectParser(TokenBuffer tokenBuf, int k) {
  super(tokenBuf,k);
...
public Sql92SelectParser(TokenBuffer tokenBuf) {
  this(tokenBuf,1);
}

protected Sql92SelectParser(TokenStream lexer, int k) {
  super(lexer,k);
  tokenNames = _tokenNames;
  buildTokenTypeASTClassMap();
  astFactory = new ASTFactory(getTokenTypeToASTClassMap());
...
public Sql92SelectParser(TokenStream lexer) {
  this(lexer,1);
}

public Sql92SelectParser(ParserSharedInputState state) {
  super(state,1);
  tokenNames = _tokenNames;
  buildTokenTypeASTClassMap();
  astFactory = new ASTFactory(getTokenTypeToASTClassMap());
...
inputState.guessing--;
	...
	public final void column_alias() throws RecognitionException, TokenStreamException {
		...
		case QuotedIdentifier:
		{
			...
			astFactory.addASTChild(currentAST, returnAST);
			if ( inputState.guessing==0 ) {
				i_AST.setType(COLUMN_IDENTIFIER_ALIAS);
			}
			break;
		}
		case UnicodeStringLiteral:
		case ASCIIStringLiteral:
		{
			...
			astFactory.addASTChild(currentAST, returnAST);
			if ( inputState.guessing==0 ) {
				s_AST.setType(COLUMN_IDENTIFIER_ALIAS);
			}
			break;
		}
		default:
...
inputState.guessing--;
	...
	public final void condition() throws RecognitionException, TokenStreamException {
		...
		if(stateSQL != STATE_JOIN_CONDITION){stateSQL = STATE_CONDITION;}
