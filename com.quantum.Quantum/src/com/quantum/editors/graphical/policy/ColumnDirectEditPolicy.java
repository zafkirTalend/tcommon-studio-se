...
import com.quantum.editors.graphical.model.commands.ResetAliasForColumnCommand;
import com.quantum.editors.graphical.model.commands.ResetNameForColumnCommand;
...
public class ColumnDirectEditPolicy extends DirectEditPolicy
{
	...
	{
        ColumnPart tp = (ColumnPart) getHost();
        if(tp.isNameBeingEdited())
        {
    		ResetNameForColumnCommand cmd = new ResetNameForColumnCommand();
            ...
            ResetAliasForColumnCommand cmd = new ResetAliasForColumnCommand();
            Column column = (Column) getHost().getModel();
            cmd.setSource(column);
            cmd.setOldName(column.getAliasName());
            CellEditor cellEditor = request.getCellEditor();
            cmd.setNameType((String) cellEditor.getValue());
            return cmd;
        }
