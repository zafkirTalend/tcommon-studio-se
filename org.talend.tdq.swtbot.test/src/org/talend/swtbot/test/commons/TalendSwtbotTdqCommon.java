...
public class TalendSwtbotTdqCommon {
	...
	public static void createFileDelimitedConnection(SWTWorkbenchBot bot,
		...
		SWTBotTreeItem newCsvItem = tree
				.expandNode("Metadata", "fileDelimited").select(fileName);
	...
	//	bot.sleep(1000);
		...
		try {
			...
			case REPORT:
				...
				break;
			case FILE_DELIMITED:
				tree.expandNode("Metadata", "fileDelimited").select(label);
				break;
			case MDM:
				tree.expandNode("Metadata", "MDMconnections").select(label);
				break;
			case LIBRARY_DQRULE:
