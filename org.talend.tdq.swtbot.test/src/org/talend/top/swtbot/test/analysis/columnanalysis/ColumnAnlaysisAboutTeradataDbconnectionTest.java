...
public class ColumnAnlaysisAboutTeradataDbconnectionTest extends TalendSwtbotForTdq{
	
	@Before
	public void beforeClass(){
		
		TalendSwtbotTdqCommon.createConnection(bot,
				TalendMetadataTypeEnum.TERADATA);
		
		bot.editorByTitle(TalendMetadataTypeEnum.TERADATA.toString() + " 0.1")
				.close();
	...
	@Test
	public void ColumnAnlaysisAboutTeradataDbconnection(){
		
		String item = bot.tree().expandNode("Metadata","DB connections","TERADATA", "talenddb","Tables (0)","myTable","Columns (2)").getNodes().get(1);
		...
		tree.expandNode("talenddb").getNode(0).expand().select("myTable");
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.TERADATA.toString(),"talenddb").
		getNode(0).select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.TERADATA.toString(),"talenddb").
		getNode(0).expand().getNode("myTable").expand().getNode(0).select();
