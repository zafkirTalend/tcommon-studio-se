...
public class ColumnAnlaysisAboutDb2ZosDbconnectionTest extends TalendSwtbotForTdq{
	
	@Before
	public void beforeClass(){
		
		TalendSwtbotTdqCommon.createConnection(bot,
				TalendMetadataTypeEnum.DB2ZOS);
		
		bot.editorByTitle(TalendMetadataTypeEnum.DB2ZOS.toString() + " 0.1")
				.close();
	...
	public void ColumnAnlaysisAboutDb2ZosDbconnection(){
		
		String item = bot.tree().expandNode("Metadata","DB connections","DB2ZOS", "ZHAO","Tables (0)","DEMOTABLE","Columns (2)").getNodes().get(1);
		...
		tree.expandNode("ZHAO").getNode(0).expand().getNode(0).select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.DB2ZOS.toString(),"ZHAO").
		getNode(0).select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.DB2ZOS.toString(),"ZHAO").
		getNode(0).expand().getNode(0).expand().getNode(0).select();
