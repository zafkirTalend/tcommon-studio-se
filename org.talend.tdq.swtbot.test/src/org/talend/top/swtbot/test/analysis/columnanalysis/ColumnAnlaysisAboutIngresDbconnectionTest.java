...
public class ColumnAnlaysisAboutIngresDbconnectionTest extends TalendSwtbotForTdq{
	
	@Before
	public void beforeClass(){
		
		TalendSwtbotTdqCommon.createConnection(bot,
				TalendMetadataTypeEnum.INGRES);
		
		bot.editorByTitle(TalendMetadataTypeEnum.INGRES.toString() + " 0.1")
				.close();
	...
	@Test
	public void ColumnAnlaysisAboutIngresDbconnection(){
		
		String item = bot.tree().expandNode("Metadata","DB connections","INGRES", "zhao","Tables (0)","yzhao","Columns (2)").getNodes().get(1);
		...
		tree.expandNode("zhao").getNode(0).expand().getNode("yzhao").select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.INGRES.toString(),"zhao").
		getNode(0).select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.INGRES.toString(),"zhao").
		getNode(0).expand().getNode("yzhao").expand().getNode(0).select();
