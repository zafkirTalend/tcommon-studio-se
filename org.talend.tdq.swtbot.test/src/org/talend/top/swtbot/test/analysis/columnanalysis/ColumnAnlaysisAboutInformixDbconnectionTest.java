...
public class ColumnAnlaysisAboutInformixDbconnectionTest extends TalendSwtbotForTdq{
	...
	public void beforeClass(){
		...
		TalendSwtbotTdqCommon.createConnection(bot,
				TalendMetadataTypeEnum.INFORMIX);
		
		bot.editorByTitle(TalendMetadataTypeEnum.INFORMIX.toString() + " 0.1")
				.close();
	...
	@Test
	public void ColumnAnlaysisAboutInformixDbconnection(){
		
		String item = bot.tree().expandNode("Metadata","DB connections","INFORMIX", "talend","zhao","Tables (0)","test","Columns (2)").getNodes().get(1);
		...
		tree.expandNode("talend").getNode("zhao").expand().getNode(0).expand().select("test");
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.INFORMIX.toString(),"talend").
		getNode("zhao").expand().getNode(0).select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.INFORMIX.toString(),"talend").
		getNode("zhao").expand().getNode(0).expand().getNode("test").expand().getNode(0).select();
