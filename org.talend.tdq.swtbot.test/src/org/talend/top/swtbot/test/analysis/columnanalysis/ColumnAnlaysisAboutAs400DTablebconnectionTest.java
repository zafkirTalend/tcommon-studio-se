...
public class ColumnAnlaysisAboutAs400DTablebconnectionTest extends TalendSwtbotForTdq{
	...
	@Test
	public void ColumnAnlaysisAboutAs400DTablebconnection(){
		String column = TalendSwtbotTdqCommon.getColumns(bot,
				TalendMetadataTypeEnum.AS400 , "S6581FBD", "FIDEM", "DMNM1")[0];
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.AS400.toString(),"S6581FBD").
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.AS400.toString(),"S6581FBD").
		getNode(0).expand().getNode(0).expand().getNode("FIDEM").expand().getNode(0).select();
		ContextMenuHelper.clickContextMenu(tree, "Reload column list");
		bot.waitUntil(Conditions.shellIsActive("Confirm reload"));
		bot.button("OK").click();
		bot.waitUntil(Conditions.shellIsActive("Analyzed element changed"));
