...
public class ColumnAnlaysisAboutDb2DbConnectionTest extends TalendSwtbotForTdq{
	
	@Before
	public void beforeClass(){
		
		TalendSwtbotTdqCommon.createConnection(bot,
				TalendMetadataTypeEnum.DB2);
		
		bot.editorByTitle(TalendMetadataTypeEnum.DB2.toString() + " 0.1")
				.close();
	...
	@Test
	public void ColumnAnlaysisAboutDb2DbConnection(){
		
		String item = bot.tree().expandNode("Metadata","DB connections","DB2", "ZHAO","Tables (0)","DEMOTABLE","Columns (2)").getNodes().get(1);
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.DB2.toString(),"ZHAO").
		getNode(0).select();
