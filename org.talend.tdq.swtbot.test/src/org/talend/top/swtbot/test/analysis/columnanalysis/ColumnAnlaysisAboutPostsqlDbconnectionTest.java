...
public class ColumnAnlaysisAboutPostsqlDbconnectionTest extends TalendSwtbotForTdq{
	...
	@Test
	public void ColumnAnlaysisAboutPostsqlDbconnection(){
		
		String item = bot.tree().expandNode("Metadata","DB connections","POSTGRESQL", "shenze","optest","Tables (0)","test","Columns (2)").getNodes().get(1);
		...
		tree.expandNode("shenze").getNode("optest").expand().getNode(0).expand().select("test");
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.POSTGRESQL.toString(),"shenze","optest").
		getNode(0).select();
		...
		tree.expandNode("Metadata","DB connections",TalendMetadataTypeEnum.POSTGRESQL.toString(),"shenze","optest").
		getNode(0).expand().getNode("test").expand().getNode(0).select();
