...
public class GenerateAnELTJobToHandleRowsTest extends TalendSwtbotForTdq{
	...
	@Test
	public void GenerateAnELTJobToHandleRows(){
...
//		tableItem.contextMenu("Generate Job").click();
	...
	//	SWTBotTreeItem Item =tree.expandNode("Email Adress").select();
		...
		ContextMenuHelper.clickContextMenu(bot.tableWithLabel("Pattern Matching"), "Generate Job");
		
		SWTBotShell shell = bot.shell("Job Selector");
		bot.waitUntil(Conditions.shellIsActive("Job Selector"));
		bot.radio("generate an ETL job to handle rows").click();
