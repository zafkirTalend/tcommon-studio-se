...
public class NewReportWithTableAnalysisTest extends TalendSwtbotForTdq{
	
	private final String REPORTLABEL = "reportWithTableAnalysis";
	...
	@Test
	public void newReportWithTableAnalysis(){
		System.out.println("aaaaaaaaaaa"+TalendMetadataTypeEnum.MYSQL.toString()+"nnnn");
		bot.editorByTitle(TalendAnalysisTypeEnum.DQRULE.toString()+" 0.1").show();
		...
		tree.expandNode("DB connections",TalendMetadataTypeEnum.MYSQL.toString(),"tbi").getNode(0).select();
		bot.table().getTableItem("customer").check();
		bot.button("OK").click();
		/*
		bot.button("OK").click();
		 */
		bot.editorByTitle(TalendAnalysisTypeEnum.DQRULE.toString()+" 0.1").setFocus();
	...
	TalendSwtbotTdqCommon.generateReport(bot, formBot, REPORTLABEL, TalendReportTemplate.Evolution, TalendAnalysisTypeEnum.DQRULE.toString());
