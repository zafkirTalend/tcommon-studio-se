 ...
 * </ul>
 * <!-- end-user-doc --> <!-- begin-model-doc --> The Warehouse Operation
 * package contains classes recording the day-to-day operation of the warehouse
 * processes.
 * 
 * The package covers three separate areas:
 * 
 * Transformation Executions Measurements Change Requests
 * 
 * Details of the most recent executions of transformations can be recorded,
 * identifying when they ran and whether they completed successfully. This can
 * be used to determine how complete and up-to-date specific information in the
 * data warehouse is.
 * 
 * An ActivityExecution represents an execution of a whole
 * TransformationActivity, and a StepExecution object represents an execution of
 * an individual TransformationStep. If a TransformationStep involves the use of
 * an Operation , an associated StepExecution may reference a CallAction that
 * records the actual arguments passed to the Operation. These classes allow the
 * lineage of data in a data warehouse to be preserved, by recording when and
 * how it was derived, and where it came from.
 * 
 * Measurement objects allow metrics to be held for any ModelElement. For
 * example, they may be used to hold actual, estimated or planned values for the
 * size of a table.
 * 
 * ChangeRequests allow details of proposed changes affecting any ModelElement
 * to be recorded. They may also be used to keep a historical record of changes
 ...
 * 
 * Separate model diagrams are shown below for each of the three main areas
 * supported by the package.
 * 
 * OCL Representation of Warehouse Operation Constraints
 * 
 * [C-1] A ChangeRequest instance must not apply to itself. context
 * ChangeRequest inv: self.modelElement -> forAll (element | element <> self)
 * 
 * [C-2] A completionDate may only be provided for a completed ChangeRequest.
 * context ChangeRequest inv: self.completionDate->notEmpty implies
 * self.completed
 * 
 * [C-3] A Measurement instance must not apply to itself. context Measurement
 * inv: self.modelElement <> self
 * 
 * [C-4] If the TransformationExecution is not inProgress, the successful,
 * status and endDate attributes must be present, and endDate must not be
 * earlier than startDate. context TransformationExecution inv:
 * self.inProgress=false implies (self.successful->notEmpty and
 * self.status->notEmpty and self.endDate->notEmpty and self.endDate >=
 * self.startDate)
 * 
 * <!-- end-model-doc -->
 * 
...
public interface WarehouseoperationPackage extends EPackage {
     ...
     * {@link orgomg.cwm.management.warehouseoperation.impl.MeasurementImpl
     ...
     * {@link orgomg.cwm.management.warehouseoperation.ChangeRequest#getChangeDescription
     * <em>Change Description</em>}'. <!-- begin-user-doc --> <!-- end-user-doc
     * -->
     * 
     ...
     * {@link orgomg.cwm.management.warehouseoperation.ActivityExecution
