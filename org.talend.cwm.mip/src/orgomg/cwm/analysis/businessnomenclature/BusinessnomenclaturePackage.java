 ...
 * </ul>
 * <!-- end-user-doc --> <!-- begin-model-doc --> Business users of data
 * warehouses need to have a good understanding of what information and tools
 * exist in a data warehouse. They need to understand what the information means
 * from a business perspective, how it is derived, from what data resources it
 * is derived, and what analysis and reporting tools exist for manipulating and
 * reporting the information. They may also need to subscribe to analysis and
 * reporting tools, and have them run with results delivered to them on a
 * regular basis.
 * 
 * The BusinessNomenclature package contains classes and associations that can
 * be used to represent business metadata. Easy access to this business metadata
 * enables business users to exploit the value of the information in a data
 * warehouse. It can also aid technical users in certain tasks. An example is
 * the use of common business terms and concepts for discussing information
 * requirements with business users. Another example is accessing business
 * intelligence tools for analyzing the impact of warehouse design changes.
 * 
 * The scope of the BusinessNomenclature package is restricted to the domain of
 * data warehousing and business intelligence.
 * 
 * This section provides a description of the main features of the
 * BusinessNomenclature package.
 * 
 * The BusinessNomenclature package provides two main constructs to represent
 * business terms and concepts and related semantics:
 * 
 * Taxonomy is a collection of concepts that provide the context for the meaning
 * of a particular term. Glossary is a collection of terms and various related
 * forms of the term.
 * 
 * A taxonomy is a collection of concepts. Concepts represent semantic
 * information and relationships. Concepts are identified by terms, which in
 * turn are manifested by a word or phrase. More than one term may describe the
 * same concept and a given term may describe more than one concept. A glossary
 * is a collection of terms that are related through explicit or implicit
 * relationships. Terms may be preferred (the term best representing its
 * concept) and thus represent the vocabulary of a business domain or user.
 * Terms may be synonyms and point at the preferred term. A preferred term and
 * its synonyms represent the fact that several terms describe the same concept
 * although with different shades of meaning.
 * 
 * Terms may be arranged into a hierarchy of more generic and more specific
 * elements. This relationship allows substituting a narrower term, such as
 * "USA", for a wider term, such as "country".
 * 
 * The BusinessNomenclature package depends on the following packages:
 * 
 * omg.org::CWM::ObjectModel::Core
 * 
 * The metamodel diagram for the BusinessNomenclature package is split into two
 * parts. The first diagram shows the BusinessNomenclature classes and
 * associations, while the second shows the inheritance hierarchy.
 * 
 * OCL Representation of Business Nomenclature Constraints [C-1] A Concept may
 * not relate to itself. context Concept inv: self.relatedConcept->forAll ( p |
 * p <> self )
 * 
 * [C-2] The parent of a Glossary must be a Glossary. context Glossary inv:
 * self.parent.oclIsKindOf( Glossary )
 * 
 * [C-3] The child of a Glossary must be a Glossary. context Glossary inv:
 * self.child->forAll( p | p.oclIsKindOf( Glossary ) )
 * 
 * [C-4] The parent of a Taxonomy must be a Taxonomy. context Taxonomy inv:
 * self.parent.oclIsKindOf( Taxonomy )
 * 
 * [C-5] The child of a Taxonomy must be a Taxonomy. context Taxonomy inv:
 * self.child->forAll( p | p.oclIsKindOf( Taxonomy ) )
 * 
 * [C-6] A Term may not relate to itself. context Term inv:
 * self.relatedTerm->forAll ( p | p <> self )
 * 
 * [C-7] A VocabularyElement may not relate to itself. context Vocabulary inv:
 * self.relatedElement->forAll ( p | p <> self )
 * 
 * [C-8] The RelatedConcepts association is derived from the
 * RelatedVocabularyElements association. All ends of the RelatedConcepts
 * association must be Concepts. context RelatedConcepts inv:
 * RelatedVocabularyElements.allInstances.select( element.oclIsKindOf( Concept )
 * and relatedElement.oclIsKindOf( Concept ) )
 * 
 * [C-9] The RelatedTerms association is derived from the
 * RelatedVocabularyElements association. All ends of the RelatedTerms
 * association must beTerms. context RelatedTerms inv:
 * RelatedVocabularyElements.allInstances.select( element.oclIsKindOf( Term )
 * and relatedElement.oclIsKindOf( Term ) ) <!-- end-model-doc -->
 * 
...
public interface BusinessnomenclaturePackage extends EPackage {
     ...
     * {@link orgomg.cwm.analysis.businessnomenclature.impl.NomenclatureImpl
     * <em>Nomenclature</em>}' class. <!-- begin-user-doc --> <!-- end-user-doc
     * -->
     * 
     ...
     * {@link orgomg.cwm.analysis.businessnomenclature.VocabularyElement
    ...
    interface Literals {
         ...
         * {@link orgomg.cwm.analysis.businessnomenclature.impl.TaxonomyImpl
         * <em>Taxonomy</em>}' class. <!-- begin-user-doc --> <!-- end-user-doc
         * -->
         * 
         * @see orgomg.cwm.analysis.businessnomenclature.impl.TaxonomyImpl
         * @see orgomg.cwm.analysis.businessnomenclature.impl.BusinessnomenclaturePackageImpl#getTaxonomy()
        ...
        /**
         * The meta object literal for the '
         * {@link orgomg.cwm.analysis.businessnomenclature.impl.GlossaryImpl
        ...
        /**
         * The meta object literal for the '
         * {@link orgomg.cwm.analysis.businessnomenclature.impl.ConceptImpl
