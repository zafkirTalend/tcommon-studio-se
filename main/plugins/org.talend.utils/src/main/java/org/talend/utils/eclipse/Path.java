...
/**
 * The standard implementation of the <code>IPath</code> interface. Paths are always maintained in canonicalized form.
 * That is, parent references (i.e., <code>../../</code>) and duplicate separators are resolved. For example,
 * 
 * <pre>
...
public class Path implements IPath, Cloneable {
    ...
    /**
     * Constructs a new path from the given string path. The string path must represent a valid file system path on the
     * local file system. The path is canonicalized and double slashes are removed except at the beginning. (to handle
     * UNC paths). All forward slashes ('/') are treated as segment delimiters, and any segment and device delimiters
     * for the local file system are also respected.
     * 
     ...
     * local file system. The path is canonicalized and double slashes are removed except at the beginning. (to handle
     * UNC paths). All forward slashes ('/') are treated as segment delimiters, and any segment and device delimiters
     * for the local file system are also respected (such as colon (':') and backslash ('\') on some file systems).
     * 
    ...
    /**
     * Constructs a new path from the given device id and string path. The given string path must be valid. The path is
     * canonicalized and double slashes are removed except at the beginning (to handle UNC paths). All forward slashes
     * ('/') are treated as segment delimiters, and any segment delimiters for the local file system are also respected
    ...
    /**
     * Removes duplicate slashes from the given path, with the exception of leading double slash which represents a UNC
     * path.
     */
    ...
    /*
     * (Intentionally not included in javadoc) Returns the size of the string that will be created by toString or
     * toOSString.
     */
    ...
    /**
     * Returns the platform-neutral encoding of the given segment onto the given string buffer. This escapes literal
     * colon characters with double colons.
     */
