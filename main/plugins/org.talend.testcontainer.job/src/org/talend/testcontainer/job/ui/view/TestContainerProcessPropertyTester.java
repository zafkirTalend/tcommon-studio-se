...
public class TestContainerProcessPropertyTester extends AbstractNodeTester {
    ...
    private static final String IS_PROCESS_ITEM_NODE = "isProcessItemNode"; //$NON-NLS-1$
    ...
    protected Boolean testProperty(Object receiver, String property, Object[] args, Object expectedValue) {
        if (receiver instanceof RepositoryNode) {
            ...
            } else if (IS_PROCESS_ITEM_NODE.equals(property)) {
                return isProcessItemNode(repositoryNode);
            }
        }
        return null;
    }
    ...
    public boolean isProcessItemNode(RepositoryNode repositoryNode) {
        if (isTypeNode(repositoryNode, ERepositoryObjectType.PROCESS)) {
            if (repositoryNode.getType() == ENodeType.REPOSITORY_ELEMENT) {
                IRepositoryViewObject object = repositoryNode.getObject();
                if (object != null) { // must be item.
                    return true;
                }
            }
        }
