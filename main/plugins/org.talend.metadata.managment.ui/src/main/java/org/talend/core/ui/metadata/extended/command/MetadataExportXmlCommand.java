...
public class MetadataExportXmlCommand extends Command {
    ...
    public void execute() {
        ...
        if (file != null) {
            if (file.exists()) {
                boolean flag = MessageDialog.openConfirm(null, Messages.getString("MetadataExportXmlCommand.title"), Messages.getString("MetadataExportXmlCommand_message")); //$NON-NLS-1$ //$NON-NLS-2$
                if (flag) {
                    excuteExportFile();
                } else {
                    return;
    ...
    /**
     * show up a confirm dialog DOC hfchang Comment method "excuteExportFile".
     */
