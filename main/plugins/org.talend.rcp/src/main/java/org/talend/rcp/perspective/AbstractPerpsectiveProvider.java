...
/**
 * created by ggu on Nov 7, 2014 Detailled comment
 *
 */
@SuppressWarnings("restriction")
public abstract class AbstractPerpsectiveProvider {
    ...
    protected abstract EModelService getEModelService();

    protected abstract MWindow getMWindow();

    protected abstract EPartService getEPartService();

    protected abstract MApplication getMApplication();

    protected MPerspectiveStack getMPerspectiveStack() {
        if (fPerspectiveStack == null && getMWindow() != null && getEModelService() != null) {
            List<MPerspectiveStack> perspStackList = getEModelService().findElements(getMWindow(), null, MPerspectiveStack.class,
                    null);
    ...
    protected void findAndCreatePerspective(String id, List<MPerspective> validPerspList) {
        final EModelService mService = getEModelService();
        final MWindow mWindow = getMWindow();
        final EPartService partService = getEPartService();
        final MApplication mApp = getMApplication();
        final MPerspectiveStack mPerspStack = getMPerspectiveStack();

        if (id != null && mPerspStack != null && mWindow != null && partService != null && mApp != null) {
            ...
            if (perspDesc != null) {
                ...
                if (mPersp == null) { // FIXME copied some form method setPerspective of class WorkbenchPage
                    ...
                    mPersp = (MPerspective) mService.cloneSnippet(mApp, perspId, mWindow);

                    if (mPersp == null) {

                        // couldn't find the perspective, create a new one
                        mPersp = mService.createModelElement(MPerspective.class);
                        ...
                        ModeledPageLayout modelLayout = new ModeledPageLayout(mWindow, mService, partService, mPersp, perspDesc,
                                workbenchPage, true);
                        factory.createInitialLayout(modelLayout);
                        PerspectiveTagger.tagPerspective(mPersp, mService);
                    ...
                    mService.hideLocalPlaceholders(mWindow, mPersp);
     ...
     * 
     * check the perspectives for perspective bar.
     */
    public abstract void checkPerspectiveDisplayItems();
}
