...
public class LocalComponentsInstallComponent implements ComponentsInstallComponent {
        ...
        if (updateFiles != null && updateFiles.length > 0) {
            for (File f : updateFiles) {
                ...
                if (f.isFile() && UpdatesHelper.isComponentUpdateSite(f)) {
                    try {
                        ComponentIndexBean b = new ComponentIndexManager().create(f);
                        ComponentP2ExtraFeature feature = new ComponentP2ExtraFeature(b.getName(), b.getVersion(),
    ...
    public void analyzeInstalledStatus(IStatus status) {
        ...
        case IStatus.ERROR:
        case IStatus.CANCEL:
            failureMessage = failureMessage.concat(System.getProperty("line.separator")).concat(status.getMessage()); //$NON-NLS-1$
            break;
        default:
            installedMessage = installedMessage.concat(System.getProperty("line.separator")).concat(status.getMessage()); //$NON-NLS-1$
            break;
        }
    }
