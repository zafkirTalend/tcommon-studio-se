...
public class MetadataManager {
	...
	public static void getTypedPackges(ConnectionItem item, List returnlist,
			EPackageType pkgType) {
		...
		case Generic:
			for (int i = 0; i < item.getConnection().getDataPackage().size(); i++) {
				if (item.getConnection().getDataPackage().get(i) instanceof GenericPackage) {
					GenericPackage gpkg = (GenericPackage) item.getConnection()
		...
		case RecordFile:
			for (int i = 0; i < item.getConnection().getDataPackage().size(); i++) {
				if (item.getConnection().getDataPackage().get(i) instanceof RecordFile) {
					RecordFile rf = (RecordFile) item.getConnection()
							.getDataPackage().get(i);
					returnlist.add(rf);
				}
			}
