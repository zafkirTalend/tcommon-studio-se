...
/**
 * Node used to fill the repository view TreeViewer. Each node has a type
 * defines in ENodeType enum. Object isn't stored in the node but retrieve using
 * the provider at each getObject call.<br/>
 * 
 * $Id: RepositoryNode.java 914 2006-12-08 08:28:53 +0000 (星期五, 08 �??二月 2006)
 * bqian $
 * 
 */
public class RepositoryNode implements IRepositoryNode, IActionFilter {
	 ...
	 * @param type
	 */
	public RepositoryNode(IRepositoryViewObject object, RepositoryNode parent,
			ENodeType type) {
		super();
	...
	public List<IRepositoryNode> getChildren() {
		if (true) {
			...
			IBrandingService service = (IBrandingService) GlobalServiceRegister
					.getDefault().getService(IBrandingService.class);

			List<IRepositoryNode> hiddens = service.getBrandingConfiguration()
					.getHiddenRepositoryCategory(this, "DI");
		...
		if (type == ENodeType.STABLE_SYSTEM_FOLDER
		...
		for (IRepositoryViewObject currentChild : getObject().getChildren()) {
			RepositoryNode repositoryNode = new RepositoryNode(currentChild,
	...
	public String getLabel() {
		...
		default:
			...
			if (ERepositoryObjectType.TDQ_INDICATOR_ELEMENT
	...
	public IProjectRepositoryNode getRoot() {
		return ProjectManager.getInstance().getProjectNode(
				this.projectTechnicalLabel);
	}
	...
	/**
	 * it is used to TDQ ,some subClasses in TDQ will overwrite it. if
	 * withDeleted is true,will contain some logical delete elements; if
	 * withDeleted is false,will not catain some logical delete elements.
	 * 
	 */
	...
	public void dispose() {
		/*
		 * FIXME, if later, can dispose all children, the flag enableDisposed
		 * will be not useful. So far, because many actions reference this
		 * object, so can't dispose it, only dispose the StableRepositoryNode.
		 */
		...
		if (this.children != null) {
			for (IRepositoryNode child : this.children) {
				/*
				 * In fact, won't dispose the children. because the flag
				 * 'enableDisposed' is false always. So far, only dispose the
				 * StableRepositoryNode.
				 */
				child.dispose();
			}
