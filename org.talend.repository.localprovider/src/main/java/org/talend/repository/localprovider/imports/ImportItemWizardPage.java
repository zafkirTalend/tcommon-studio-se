...
class ImportItemWizardPage extends WizardPage {
	...
	private String[] getAvailableItems() {
		boolean jobTemplateEnabled = (Platform
				.getBundle("org.talend.designer.jobtemplate") == null ? false
				: true);
		ERepositoryObjectType types[] = {
				ERepositoryObjectType.BUSINESS_PROCESS,
				ERepositoryObjectType.PROCESS, ERepositoryObjectType.JOBLET,
	...
	private void createErrorsList(Composite workArea) {
		...
		title.setText(Messages
				.getString("ImportItemWizardPage.ErrorsAndWarnings")); //$NON-NLS-1$
	...
	private void createItemList(Composite workArea) {
		...
		itemListInfo.setText(Messages
				.getString("ImportItemWizardPage.NoValidItems")); //$NON-NLS-1$
	...
	private TreeViewer createTreeViewer(Composite listComposite) {
		filteredCheckboxTree = new FilteredCheckboxTree(listComposite,
		...
		viewer.setContentProvider(new ITreeContentProvider() {
			...
			public Object[] getChildren(Object parentElement) {
				if (parentElement instanceof IContainerNode) {
					return ((IContainerNode) parentElement).getChildren()
							.toArray();
				}
				return null;
			}
				...
				public void run(IProgressMonitor monitor) {
					...
					if (!dirSelected
						...
						TarLeveledStructureProvider provider = new TarLeveledStructureProvider(
								sourceTarFile);
						manager = ResourcesManagerFactory.getInstance()
	...
	public ItemRecord[] getValidItems() {
		...
		return (ItemRecord[]) validItems.toArray(new ItemRecord[validItems
				.size()]);
	}
		...
		try {
			IRunnableWithProgress op = new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor)
					...
					List<ItemRecord> importItemRecords = repositoryUtil
							.importItemRecords(manager, itemRecords, monitor,
									overwrite);

					if (repositoryUtil.hasErrors()) {
						throw new InvocationTargetException(
								new PersistenceException("")); //$NON-NLS-1$
					}
				}
			...
			if (targetException instanceof PersistenceException) {
				MessageDialog
						.openWarning(
								...
								Messages
										.getString("ImportItemWizardPage.ErrorsOccured")); //$NON-NLS-1$
			}
		} catch (InterruptedException e) {
			//
		}
