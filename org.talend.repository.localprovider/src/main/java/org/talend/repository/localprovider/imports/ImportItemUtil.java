...
public class ImportItemUtil {
	...
	private static IPath getPath(RepositoryNode node) {
		...
		if (node.getType() == ENodeType.STABLE_SYSTEM_FOLDER
			...
			String prefix = "";
			ERepositoryObjectType type = (ERepositoryObjectType) node
	...
	private boolean checkItem(ItemRecord itemRecord, boolean overwrite) {
		...
		try {
			boolean nameAvailable = ProxyRepositoryFactory.getInstance()
					.isNameAvailable(itemRecord.getItem(),
							itemRecord.getProperty().getLabel());
			org.talend.core.model.general.Project project = projectManager
					.getCurrentProject();
			IRepositoryObject itemWithSameId = ProxyRepositoryFactory
					.getInstance().getLastVersion(project,
							itemRecord.getProperty().getId());
			boolean idAvailable = itemWithSameId == null;

			boolean isSystem = false;
			// we do not import built in routines
			if (itemRecord.getItem().eClass().equals(
					PropertiesPackage.eINSTANCE.getRoutineItem())) {
			...
			if (itemRecord.getItem().eClass().equals(
					PropertiesPackage.eINSTANCE.getSQLPatternItem())) {
				SQLPatternItem sqlPatternItem = (SQLPatternItem) itemRecord
						.getItem();
			...
			if (nameAvailable) {
				if (idAvailable) {
					if (!isSystem) {
						...
						itemRecord.addError(Messages
								.getString("RepositoryUtil.isSystemRoutine")); //$NON-NLS-1$ 
					}
				} else {
					...
					if (overwrite) {
						...
						RepositoryNode nodeWithSameId = RepositoryNodeUtilities
								.getRepositoryNode(itemWithSameId);
						IPath path = getPath(nodeWithSameId);
						itemRecord
								.addError(Messages
										.getString("RepositoryUtil.idUsed") + " by item '" + itemWithSameId.getLabel() + "' in '" + path.toOSString() + "'"); //$NON-NLS-1$
					}
				}
			} else {
				if (idAvailable) {
				...
				if (!result) {
					itemRecord.addError(Messages
							.getString("RepositoryUtil.nameUsed")); //$NON-NLS-1$
				}
			}
			...
			if (result && overwrite
				...
				if (checkIfLocked(itemRecord)) {
					itemRecord.addError(Messages
							.getString("RepositoryUtil.itemLocked")); //$NON-NLS-1$
					result = false;
				}
			}
		} catch (Exception e) {
	 ...
	 */
	private boolean checkIfLocked(ItemRecord itemRecord)
			throws PersistenceException {
	...
	public List<ItemRecord> importItemRecords(final ResourcesManager manager,
			final List<ItemRecord> itemRecords, final IProgressMonitor monitor,
			final boolean overwrite) {
		monitor
				.beginTask(
						Messages
								.getString("ImportItemWizardPage.ImportSelectedItems"), itemRecords.size() + 1); //$NON-NLS-1$
			...
			public void run() throws PersistenceException {
				for (ItemRecord itemRecord : itemRecords) {
					if (!monitor.isCanceled()) {
						monitor
								.subTask(Messages
										.getString("ImportItemWizardPage.Importing") + itemRecord.getItemName()); //$NON-NLS-1$
				...
				monitor
						.beginTask(
								Messages
										.getString("ImportItemWizardPage.ApplyMigrationTasks"), itemRecords.size() + 1); //$NON-NLS-1$
				for (ItemRecord itemRecord : itemRecords) {
	...
	private void reinitRepository() {
		ProxyRepositoryFactory repFactory = ProxyRepositoryFactory
				.getInstance();
		if (usedItems++ > 2) {
			usedItems = 0;
			try {
	...
	private void importItemRecord(ResourcesManager manager,
			ItemRecord itemRecord, boolean overwrite) {
		resolveItem(manager, itemRecord);
		if (itemRecord.getItem() != null) {
			ERepositoryObjectType itemType = ERepositoryObjectType
					.getItemType(itemRecord.getItem());
			...
			try {
				...
				if (overwrite && !itemRecord.isLocked()
						&& itemRecord.getState() == State.ID_EXISTED
						&& !deletedItems.contains(id)) {
					List<IRepositoryObject> list = cache
							.findObjectsByItem(itemRecord);
					if (!list.isEmpty()) {
				...
				org.talend.core.model.general.Project project = projectManager
						.getCurrentProject();
				IRepositoryObject lastVersion = repFactory.getLastVersion(
						project, tmpItem.getProperty().getId());

				User author = itemRecord.getProperty().getAuthor();
				if (author != null) {
					if (!repFactory
							.setAuthorByLogin(tmpItem, author.getLogin())) {
				...
				} else if (VersionUtils.compareTo(lastVersion.getProperty()
						.getVersion(), tmpItem.getProperty().getVersion()) < 0) {
					repFactory.forceCreate(tmpItem, path);
					changeRoutinesPackage(tmpItem);
					itemRecord.setImported(true);
				} else {
					PersistenceException e = new PersistenceException(
							"A newer version of " + tmpItem.getProperty()
									+ " already exist.");
	...
	public void changeRoutinesPackage(Item item) {
		...
		ERepositoryObjectType itemType = ERepositoryObjectType
				.getItemType(item);
		if (ERepositoryObjectType.ROUTINES.equals(itemType)
				&& item instanceof RoutineItem) {
			RoutineItem rItem = (RoutineItem) item;
			if (!rItem.isBuiltIn()) {
				//
				String routineContent = new String(rItem.getContent()
						.getInnerContent());
				ProjectManager pManager = ProjectManager.getInstance();
				org.talend.core.model.general.Project currentProject = pManager
						.getCurrentProject();
				//
				...
				try {
					...
					if (matcher.contains(routineContent, pattern)) {
						...
						Perl5Substitution substitution = new Perl5Substitution(
								newPackage, Perl5Substitution.INTERPOLATE_ALL);
						routineContent = Util.substitute(matcher, pattern,
								substitution, routineContent,
								Util.SUBSTITUTE_ALL);

						rItem.getContent().setInnerContent(
								routineContent.getBytes());
						ProxyRepositoryFactory repFactory = ProxyRepositoryFactory
								.getInstance();
	...
	private void applyMigrationTasks(ItemRecord itemRecord,
		...
		Item item = null;
		try {
			item = ProxyRepositoryFactory.getInstance().getUptodateProperty(
					itemRecord.getItem().getProperty()).getItem();
		} catch (Exception e) {
			logError(e);
		}
		...
		for (String taskId : itemRecord.getMigrationTasksToApply()) {
			...
			if (task == null) {
				log.warn("Task " + taskId
						+ " found in project doesn't exist anymore !");
			} else {
				monitor.subTask("apply migration task " + task.getName()
						+ " on item " + itemRecord.getItemName());

				try {
					if (item != null) {
						ExecutionResult executionResult = task.execute(
								repositoryContext.getProject(), item);
						if (executionResult == ExecutionResult.FAILURE) {
							log.warn("Incomplete import item "
									+ itemRecord.getItemName()
									+ " (migration task " + task.getName()
									+ " failed)");
					...
					log.warn("Incomplete import item "
							+ itemRecord.getItemName() + " (migration task "
							+ task.getName() + " failed)");
				}
			}
		}
	...
	private void logError(Exception e) {
		...
		status = new Status(IStatus.ERROR,
				RepositoryLocalProviderPlugin.PLUGIN_ID, IStatus.OK,
				messageStatus, e);
	...
	public List<ItemRecord> populateItems(ResourcesManager collector,
			boolean overwrite, IProgressMonitor progressMonitor) {
		treeBuilder.clear();
		cache.clear();
				...
				if (collector.getPaths().contains(itemPath)) {
					...
					if (property != null) {
						...
						if (checkItem(itemRecord, overwrite)) {
							...
							IPath projectFilePath = getValidProjectFilePath(
									collector, path, uri);
							if (projectFilePath != null) {
								Project project = computeProject(collector,
										itemRecord, projectFilePath);
								...
								itemRecord
										.addError(Messages
												.getString("RepositoryUtil.ProjectNotFound")); //$NON-NLS-1$
							}
						}
					}
				}
		...
		Collections.sort(items, new Comparator<ItemRecord>() {
			...
			public int compare(ItemRecord o1, ItemRecord o2) {
				return VersionUtils.compareTo(o1.getProperty().getVersion(), o2
	...
	private List<String> getOptionnalMigrationTasks() {
		...
		toReturn
				.add("org.talend.repository.documentation.migrationtask.generatejobdocmigrationtask");
		// old task, added for an old version of TOS, not used anymore.
		toReturn
				.add("org.talend.repository.migration.ReplaceOldContextScriptCodeMigrationTask");

		return toReturn;
	}

	@SuppressWarnings("unchecked")
	private boolean checkMigrationTasks(Project project, ItemRecord itemRecord,
			Project currentProject) {
		List<String> itemMigrationTasks = new ArrayList(project
				.getMigrationTasks());
		List<String> projectMigrationTasks = new ArrayList(currentProject
				.getMigrationTasks());
		...
		if (!projectMigrationTasks.containsAll(itemMigrationTasks)) {
			...
			String message = "Cannot import item " + itemRecord.getItemName()
					+ " -> unknown task(s) " + itemMigrationTasks;
	...
	private IPath getValidProjectFilePath(ResourcesManager collector,
			IPath path, URI uri) {
	...
	private Resource createResource(ResourceSet resourceSet, IPath path,
			boolean byteArrayResource) throws FileNotFoundException {
		Resource resource;
	...
	private IPath getItemPath(IPath path) {
		return path.removeFileExtension().addFileExtension(
	...
	static class RepositoryObjectCache {
		...
		static ProxyRepositoryFactory factory = ProxyRepositoryFactory
				.getInstance();
		...
		public List<IRepositoryObject> findObjectsByItem(ItemRecord itemRecord)
				throws PersistenceException {
			...
			List<IRepositoryObject> result = cache.get(itemRecord.getProperty()
					.getId());
			if (result == null) {
